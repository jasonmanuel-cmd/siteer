...
const breadcrumbSchema = {
    ...
    itemListElement: [
        ...
        { "@type": "ListItem", position: 3, name: "How to Fix Slow Mobile Pages in Under an Hour", item: "https://siteer.dev/blog/mobile-page-speed-fixes" },
    ],
};
                ...
                <article>
                    ...
                    <section style={{ borderRadius: 24, padding: 28, background: "rgba(255,255,255,.065)", border: "1px solid rgba(255,255,255,.13)", marginBottom: 24 }}>
                        ...
                        <ol style={{ display: "grid", gap: 14, listStyle: "none", padding: 0, margin: 0 }}>
                            {[
                                "Compress the biggest images and convert them to AVIF/WebP.",
                                "Delay non-essential scripts until after the first visible paint.",
                                "Use a single CTA in the hero and keep the form short.",
                                "Remove layout jumps by giving media and cards fixed dimensions.",
                                "Keep the page layout column-based on narrow screens.",
                            ].map((item, i) => (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 18, marginBottom: 48 }}>
                        {[
                            ["⚡ Image hygiene", "The biggest wins often come from shrinking hero media and replacing unnecessary screenshots with text or SVG."],
                            ["🧹 Script hygiene", "Client-side code should be reserved for the parts of the page that truly need it. Audit every script tag."],
                            ["📐 Spacing hygiene", "Avoid massive vertical gaps that push conversion elements below the fold on phones."],
                            ["📣 CTA hygiene", "Make the call to action visible without scrolling or pinching. One strong CTA beats three weak ones."],
                        ].map(([title, desc]) => (
                            <section key={title as string} style={{ borderRadius: 20, padding: 22, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.1)" }}>
                    ...
                    <section style={{ borderRadius: 24, padding: 28, background: "linear-gradient(135deg, rgba(255,77,94,.1), rgba(255,177,92,.06))", border: "1px solid rgba(255,77,94,.2)", marginBottom: 48 }}>
                        ...
                        <p style={{ color: "var(--er-muted)", fontSize: "0.95rem", marginBottom: 20, lineHeight: 1.65 }}>Run a free scan to see exactly which speed issues are on your page. We'll quantify the revenue impact and, if you want, fix everything for a flat $497.</p>
